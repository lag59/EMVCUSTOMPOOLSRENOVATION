...
let index=0,timer,touchStart=0,isPaused=matchMedia('(prefers-reduced-motion: reduce)').matches;
...
function show(next,restart=true){const max=Math.max(0,slides.length-visible());index=Math.max(0,Math.min(max,next));const width=slides[0]?.getBoundingClientRect().width||0;track.style.transform=`translateX(-${index*(width+12.8)}px)`;dots.forEach((d,i)=>d.classList.toggle('active',i===index));if(restart)start()}
function start(){clearInterval(timer);if(!isPaused)timer=setInterval(()=>{const max=Math.max(0,slides.length-visible());show(index>=max?0:index+1,false)},3800)}
...
windowEl?.addEventListener('mouseenter',()=>clearInterval(timer));windowEl?.addEventListener('mouseleave',start);windowEl?.addEventListener('touchstart',e=>touchStart=e.changedTouches[0].screenX,{passive:true});windowEl?.addEventListener('touchend',e=>{const distance=e.changedTouches[0].screenX-touchStart;if(Math.abs(distance)>45)show(index+(distance<0?1:-1))},{passive:true});
